...
const EmptyResult: React.FC<EmptyResultProps> = ({iconSize = 100, text, style = {}, children, iconName='layers-search', title, className}) => {
  return (
    <View style={[styles.container, style]} className={className}>
      ...
      <AmText variant="titleMedium" className="text-gray-500 mt-2 text-xl font-semibold">{title}</AmText>
      {text && <AmText variant="bodyLarge" className="text-gray-400 mt-2 text-center text-sm font-semibold">{text}</AmText>}
      {children}
    </View>
  );
};
